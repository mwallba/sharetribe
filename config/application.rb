...
module Kassi
  class Application < Rails::Application
    ...
    if APP_CONFIG.user_assets_host
      paperclip_options[:url] = "#{APP_CONFIG.user_assets_host}#{paperclip_options[:url]}"
    end
    ...
    if (APP_CONFIG.s3_bucket_name && APP_CONFIG.aws_access_key_id && APP_CONFIG.aws_secret_access_key)
      ...
      if APP_CONFIG.user_assets_host
        ...
        _, assets_proto, assets_host = *APP_CONFIG.user_assets_host.match(/^(https?):\/\/(.*)$/)
